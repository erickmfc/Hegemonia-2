...
// === COMANDOS DE TESTE PARA EDIFÍCIOS ===
// Tecla F1: Diagnóstico completo do sistema de edifícios
if (keyboard_check_pressed(vk_f1)) {
    show_debug_message("🔍 EXECUTANDO DIAGNÓSTICO COMPLETO DE EDIFÍCIOS...");
    scr_diagnostico_edificios_completo();
}

// Tecla F2: Teste de menus de edifícios
if (keyboard_check_pressed(vk_f2)) {
    show_debug_message("🧪 EXECUTANDO TESTE DE MENUS DE EDIFÍCIOS...");
    scr_teste_menus_edificios();
}

// Tecla F3: Teste de coordenadas
if (keyboard_check_pressed(vk_f3)) {
    show_debug_message("📍 TESTE DE COORDENADAS:");
    var _coords = global.scr_mouse_to_world();
    show_debug_message("   Mouse GUI: (" + string(mouse_x) + ", " + string(mouse_y) + ")");
    show_debug_message("   Mouse Mundo: (" + string(_coords[0]) + ", " + string(_coords[1]) + ")");
    show_debug_message("   Zoom Level: " + string(zoom_level));
    show_debug_message("   Camera: (" + string(camera_x) + ", " + string(camera_y) + ")");
}
...
if (!variable_global_exists("scr_mouse_to_world")) {
    ...
    global.scr_mouse_to_world = function() {
        // Obter informações da câmera atual
        ...
        // Calcular zoom
        var _zoom_x = _cam_w / display_get_gui_width();
        var _zoom_y = _cam_h / display_get_gui_height();
        
        // Converter coordenadas
        var world_x = _cam_x + (mouse_x * _zoom_x);
        var world_y = _cam_y + (mouse_y * _zoom_y);
...
if (instance_exists(global.construindo_edificio)) {
    ...
    if (mouse_check_button_pressed(mb_left) && global.construindo_agora != noone) {
        ...
        if (mouse_y < _menu_y) {
            ...
            if (_edificio_clicado == noone) {
                _edificio_clicado = collision_point(_world_mouse_x, _world_mouse_y, obj_research_center, false, true);
            }
