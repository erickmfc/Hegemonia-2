...
if (mouse_check_button_pressed(mb_left) && _clique_detectado) {
    show_debug_message("✅ CLIQUE NO BANCO DETECTADO!");
    
    // Por enquanto, apenas mostrar informações do banco
    show_debug_message("🏦 Banco clicado - ID: " + string(id));
    show_debug_message("📍 Posição: (" + string(x) + ", " + string(y) + ")");
    
    // TODO: Implementar funcionalidade específica do banco
    // Por exemplo: mostrar saldo, transações, empréstimos, etc.
}
