...
/// @description Desseleciona todas as unidades quando clica em edifício
/// @return {void}
...
/// @description Detecta se o clique foi no edifício atual
/// @return {Boolean} true se o clique foi detectado no edifício
...
function scr_detectar_clique_edificio() {
    ...
    if (mouse_check_button_pressed(mb_left) && _clique_detectado) {
        ...
        show_debug_message("🏢 Edifício: " + object_get_name(object_index) + " ID: " + string(id));
        return true;
    } else {
        show_debug_message("❌ Clique não detectado no edifício");
        return false;
    }
}