...
if (mouse_check_button_pressed(mb_left)) {
    ...
    if (!ds_exists(global.nacao_recursos, ds_type_map)) {
        show_debug_message("ERRO CRÍTICO: global.nacao_recursos não é um ds_map válido!");
        return;
    }
    
    if (!ds_exists(global.research_timers, ds_type_map)) {
        show_debug_message("ERRO CRÍTICO: global.research_timers não é um ds_map válido!");
        return;
    }
    ...
    // === DEFINIR DIMENSÕES DO MENU (EXATAMENTE IGUAL AO DRAW_64.GML MODERNO) ===
    ...
    if (point_in_rectangle(mouse_gui_x, mouse_gui_y, close_btn_x, close_btn_y, close_btn_x + close_btn_w, close_btn_y + close_btn_h)) {
        ...
        show_debug_message("Research menu closed by close button.");
        return;
    }
    ...
    if (mouse_gui_x < container_x || mouse_gui_x > container_x + container_w ||
        ...
        global.menu_pesquisa_aberto = false;
        show_debug_message("Research menu closed (clicked outside).");
        return;
    }
    
    // === 3. VERIFICAR BOTÃO SLOT EXTRA ===
    if (global.slots_pesquisa_total == 3) {
        var slot_btn_x = container_x + 30;
        ...
        if (point_in_rectangle(mouse_gui_x, mouse_gui_y, slot_btn_x, slot_btn_y, slot_btn_x + slot_btn_w, slot_btn_y + slot_btn_h)) {
            ...
            if (global.dinheiro >= global.custo_slot_extra) {
                ...
                show_debug_message("Not enough money for extra slot! Need: $" + string(global.custo_slot_extra) + ", Have: $" + string(global.dinheiro));
            }
            return;
        }
    }
    ...
    if (!variable_instance_exists(id, "research_names") || !is_array(research_names)) {
        show_debug_message("ERRO: research_names não é válido!");
        return;
    }
    ...
    for (var i = 0; i < array_length(research_names); i++) {
        ...
        if (point_in_rectangle(mouse_gui_x, mouse_gui_y, card_x, card_y, card_x + card_w, card_y + card_h)) {
            ...
            show_debug_message("*** CLIQUE DETECTADO NO CARTÃO: " + research_names[i] + " ***");
            
            var research_name = research_names[i];
            
            // Verificar se a chave existe
            if (!ds_map_exists(global.nacao_recursos, research_name)) {
                show_debug_message("ERRO: Chave '" + research_name + "' não encontrada!");
                continue;
            }
            ...
            if (status == RESOURCE_STATUS.AVAILABLE) {
                ...
                if (global.slots_pesquisa_usados < global.slots_pesquisa_total) {
                    ...
                    if (global.dinheiro >= research_cost) {
                        ...
                        show_debug_message("Dinheiro insuficiente para " + research_name + "! Precisa: $" + string(research_cost) + ", Tem: $" + string(global.dinheiro));
                    }
                } else {
                    show_debug_message("Sem slots de pesquisa disponíveis! Atual: " + string(global.slots_pesquisa_usados) + "/" + string(global.slots_pesquisa_total));
                }
            }
            else if (status == RESOURCE_STATUS.RESEARCHING) {
                show_debug_message("Pesquisa '" + research_name + "' já está em andamento.");
            }
            else if (status == RESOURCE_STATUS.RESEARCHED) {
                show_debug_message("Pesquisa '" + research_name + "' já foi concluída.");
            }
    ...
    show_debug_message("Nenhum cartão foi clicado. Mouse em: (" + string(mouse_gui_x) + ", " + string(mouse_gui_y) + ")");
}

// === TECLA ESC PARA FECHAR MENU ===
if (keyboard_check_pressed(vk_escape)) {
    global.menu_pesquisa_aberto = false;
    show_debug_message("Research menu closed (ESC key).");
}