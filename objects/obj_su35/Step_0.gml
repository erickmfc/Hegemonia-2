// ===============================================
// HEGEMONIA GLOBAL - CAÇA SU-35 (Step com Ataque Agressivo)
// ===============================================
...
if (!should_always_process && skip_frames_enabled) {
    ...
    if (!should_process) {
        // Frame skip: movimento básico apenas (SU-35 usa sistema de velocidade próprio)
...
if (selecionado) {
    // Comandos de Modo (P/O) - mesmos do F-15
    if (keyboard_check_pressed(ord("P"))) { 
        modo_ataque = false; 
        show_debug_message("🛡️ SU-35 Modo PASSIVO");
    }
    if (keyboard_check_pressed(ord("O"))) { 
        modo_ataque = true; 
        show_debug_message("⚔️ SU-35 Modo ATAQUE AGRESSIVO");
    }
...
// ======================================================================
// --- 2. DECREMENTAR TIMERS DE MÍSSEIS (4.5ª GERAÇÃO) ---
// ======================================================================
...
if (modo_ataque && estado != "pousando" && estado != "decolando" && estado != "atacando") {
    ...
    if (instance_exists(_alvo_encontrado)) {
        ...
        if (variable_global_exists("debug_enabled") && global.debug_enabled && current_time mod 5000 < 17) {
            show_debug_message("✅ SU-35 encontrou inimigo! Tipo: " + string(_alvo_encontrado.object_index));
        }
...
switch (estado) {
    ...
    case "atacando":
        ...
        if (instance_exists(alvo_em_mira)) {
            ...
            // ✅ SU-35 LANÇA MÚLTIPLOS MÍSSEIS SIMULTANEAMENTES (ar, terra, submarino e ESTRUTURAS)
            ...
            if (_alvo_valido && timer_ataque <= 0) {
                ...
                // ✅ AÉREO → SKY (4 segundos - 4.5ª geração, mais rápido que F-15)
                if (_alvo_aereo && instance_exists(alvo_em_mira) && timer_sky <= 0) {
                    ...
                    if (instance_exists(_missil_ar)) {
                        if (instance_exists(alvo_em_mira)) {
                            ...
                            _missil_ar.dono = id;
                            timer_sky = intervalo_sky;  // ✅ 4 segundos (240 frames)
                            _missois_lancados++;
                            if (variable_global_exists("debug_enabled") && global.debug_enabled) {
                                show_debug_message("✈️ SU-35 lançou Sky contra alvo aéreo");
                            }
                        } else {
                ...
                // ✅ TERRESTRE → IRON (6 segundos - 4.5ª geração)
                if (_alvo_terrestre && instance_exists(alvo_em_mira) && timer_iron <= 0) {
                    ...
                    if (instance_exists(_missil_iron)) {
                        if (instance_exists(alvo_em_mira)) {
                            ...
                            timer_iron = intervalo_iron;  // ✅ 6 segundos (360 frames)
                            _missois_lancados++;
                            if (variable_global_exists("debug_enabled") && global.debug_enabled) {
                                show_debug_message("💥 SU-35 lançou Iron contra alvo terrestre");
                            }
                        } else {
                ...
                // ✅ TERRESTRE/SUBMARINO → HASH (8 segundos - 4.5ª geração)
                if ((_alvo_terrestre || _alvo_submarino) && instance_exists(alvo_em_mira) && timer_hash <= 0) {
                    ...
                    if (instance_exists(_missil_hash) && instance_exists(alvo_em_mira)) {
                        ...
                        timer_hash = intervalo_hash;  // ✅ 8 segundos (480 frames)
                        _missois_lancados++;
                        if (variable_global_exists("debug_enabled") && global.debug_enabled) {
                            show_debug_message("💣 SU-35 lançou Hash contra alvo");
                        }
                    }
                }
                ...
                if ((_alvo_estrutura || _alvo_presidente) && instance_exists(alvo_em_mira)) {
                    ...
                    if (variable_global_exists("debug_enabled") && global.debug_enabled) {
                        show_debug_message("🏗️ SU-35 lançou " + string(_missois_lancados) + " mísseis contra estrutura/presidente!");
                    }
                }
                
                timer_ataque = intervalo_ataque;
                if (_missois_lancados > 0) {
                    show_debug_message("🚀🚀 SU-35 LANÇOU " + string(_missois_lancados) + " MÍSSEIS!");
                }
            }
        } 
