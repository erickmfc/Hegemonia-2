...
if (estado == LanchaState.MOVENDO || estado == LanchaState.PATRULHANDO || (estado == LanchaState.ATACANDO && point_distance(x, y, destino_x, destino_y) > 5)) {
    ...
    if (timer_espuma >= 3) {
        ...
        // ✅ obj_WbTrail1 no FINAL do navio (popa) - Independence: sprite 350px, origem 175px, distância ~165px
        if (object_exists(obj_WbTrail1)) {
            var _distancia_final = 165; // 350px * 0.47 ≈ 165px (proporção baseada na lancha patrulha)
