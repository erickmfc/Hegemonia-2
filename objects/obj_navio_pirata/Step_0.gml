// ===============================================
// HEGEMONIA GLOBAL - NAVIO PIRATA (Step)
...
if (!vinculado) {
    ...
    // O primeiro navio criado é o líder
    if (!variable_instance_exists(id, "eh_lider")) {
        eh_lider = false;
        // Verificar se é o líder (primeiro navio pirata mais próximo)
        var _outros_piratas = [];
        ...
        // Se não há outros piratas ou este é o mais próximo da origem, é o líder
        if (array_length(_outros_piratas) == 0 || point_distance(0, 0, x, y) < point_distance(0, 0, _outros_piratas[0].x, _outros_piratas[0].y)) {
            eh_lider = true;
            show_debug_message("🏴‍☠️ Navio Pirata é o LÍDER do esquadrão");
        }
    }
}

// === 2. SISTEMA DE FORMAÇÃO EM ESQUADRÃO ===
// ✅ NOVO: Se não é o líder, seguir o líder
if (!eh_lider) {
    // Buscar líder mais próximo
    var _lider = noone;
    ...
    if (instance_exists(_lider)) {
        // Calcular offset baseado no tipo de navio
        var _offset_angulo = 0;
        if (object_index == obj_navio_pirata2) {
            _offset_angulo = 120;  // 120 graus à direita
        } else if (object_index == obj_navio_pirata3) {
            _offset_angulo = 240;  // 240 graus à direita
        }
        ...
        if (_script_formacao != -1) {
            ...
            // Se não está atacando, seguir formação
            if (!modo_cacando) {
...
// === 3. SISTEMA DE PATRULHA CIRCULAR AO REDOR DO PILAR ===
...
if (!modo_cacando) {
    if (ds_list_size(pilares_patrulha) > 0) {
        ...
        if (instance_exists(_pilar_atual)) {
            ...
            var _raio_patrulha = 150;  // ✅ Raio de patrulha circular
            
            // ✅ NOVO: Sistema de patrulha circular
            if (!variable_instance_exists(id, "angulo_patrulha")) {
                angulo_patrulha = 0;  // Ângulo inicial
            }
            if (!variable_instance_exists(id, "tempo_patrulha_pilar")) {
                tempo_patrulha_pilar = 300;  // ✅ REDUZIDO: 5 segundos (mais rápido)
            }
            if (!variable_instance_exists(id, "timer_patrulha_pilar")) {
                timer_patrulha_pilar = tempo_patrulha_pilar;
            }
            
            // Se está dentro do raio de patrulha, circular ao redor (apenas líder)
            if (_dist_pilar <= _raio_patrulha + 50 && eh_lider) {
                // ✅ PATRULHA CIRCULAR: Navegar em círculo ao redor do pilar
                ...
                // ✅ Contar tempo patrulhando este pilar
                timer_patrulha_pilar--;
                if (timer_patrulha_pilar <= 0) {
                    // Tempo acabou - ir para próximo pilar
                    indice_pilar_atual = (indice_pilar_atual + 1) % ds_list_size(pilares_patrulha);
                    timer_patrulha_pilar = tempo_patrulha_pilar;
                    angulo_patrulha = 0;  // Resetar ângulo
                }
            } else {
                // ✅ CORREÇÃO: Sempre navegar para o pilar (líder ou seguidor)
            ...
            // Pilar destruído - remover
            ds_list_delete(pilares_patrulha, indice_pilar_atual);
            if (ds_list_size(pilares_patrulha) == 0) {
                // ✅ CORREÇÃO: Sem pilares, criar movimento aleatório imediatamente
        ...
        if (_dist_atual < 80 || estado == LanchaState.PARADO || estado == LanchaState.PATRULHANDO) {
            var _novo_x = x + random_range(-400, 400);  // ✅ AUMENTADO: Raio maior
            var _novo_y = y + random_range(-400, 400);
            destino_x = _novo_x;
            destino_y = _novo_y;
            target_x = _novo_x;
            ...
            // ✅ GARANTIR: Se está em PATRULHANDO mas não se moveu, forçar movimento
            estado = LanchaState.MOVENDO;
            is_moving = true;
            usar_novo_sistema = true;
        }
...
if (!modo_cacando || !instance_exists(alvo_atual)) {
    ...
    if (_script_buscar != -1) {
        ...
        if (instance_exists(_inimigo)) {
            ...
            is_moving = true;
            usar_novo_sistema = true;
            show_debug_message("🏴‍☠️ " + nome_unidade + " detectou inimigo: " + object_get_name(_inimigo.object_index));
        }
    }
}
...
if (modo_cacando && instance_exists(alvo_atual)) {
    ...
    // ✅ NOVO: Atacar com obj_tiro_simples se estiver no alcance
    if (_dist_alvo <= alcance_ataque) {
        reload_timer--;
        if (reload_timer <= 0) {
            // ✅ CRIAR TIRO usando obj_tiro_simples (com pool)
            ...
            if (instance_exists(_tiro) && instance_exists(alvo_atual)) {
                ...
                _tiro.speed = 8;  // Velocidade do tiro
                
                // ✅ Visual diferenciado para piratas (cor laranja/vermelha)
                _tiro.image_blend = make_color_rgb(255, 150, 0);  // Laranja
                ...
                show_debug_message("🏴‍☠️ " + nome_unidade + " atirou! Dano: " + string(_dano_final));
            }
        }
    }
    
    // Perder alvo se sair do alcance
    if (_dist_alvo > raio_deteccao * 1.5) {
        modo_cacando = false;
        alvo_atual = noone;
        estado = LanchaState.PATRULHANDO;
        // ✅ GARANTIR: Voltar a se mover após perder alvo
        is_moving = true;
        usar_novo_sistema = true;
    }
