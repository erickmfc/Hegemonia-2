...
// Sair se menu completamente fechado
...
function desenhar_botao_futurista(_x, _y, _largura, _altura, _dados_botao, _hover, _scale, _glow_intensity, _text_scale = 0.85) {
    ...
    desenhar_hexagono(_icone_x, _icone_y, _icone_tamanho, cor_borda_principal, 1.0, glow_time);
    ...
    switch (_dados_botao.icone_tipo) {
        ...
        case "quartel": _icone_char = "⚔"; break;
        case "aeroporto": _icone_char = "✈"; break;
    }
    ...
    draw_set_alpha(0.8);
    var _desc_y = _texto_y + 18;
    draw_text_transformed(_texto_x, _desc_y, _dados_botao.descricao, _text_scale, _text_scale, 0);
    
    // Custo
    draw_set_color(cor_custo);
    draw_set_alpha(0.9);
    var _custo_y = _desc_y + 16;
...
draw_set_color(make_color_rgb(0, 0, 0));
draw_set_alpha(0.5);
draw_rectangle(menu_pos_x_atual - 20, menu_pos_y - 20, menu_pos_x_atual + menu_largura + 20, menu_pos_y + menu_altura + 20, false);

// Fundo principal
draw_set_alpha(0.95);
draw_set_color(cor_fundo_principal);
draw_rectangle(menu_pos_x_atual, menu_pos_y, menu_pos_x_atual + menu_largura, menu_pos_y + menu_altura, false);

// Borda esquerda com glow animado
var _glow_val = 0.5 + (sin(glow_time * pi / 180) * 0.3);
draw_set_alpha(_glow_val);
draw_set_color(cor_borda_principal);
draw_line(menu_pos_x_atual, menu_pos_y, menu_pos_x_atual, menu_pos_y + menu_altura);
draw_line(menu_pos_x_atual + 2, menu_pos_y, menu_pos_x_atual + 2, menu_pos_y + menu_altura);

// Borda direita
draw_set_alpha(0.4);
draw_set_color(cor_borda_secundaria);
draw_line(menu_pos_x_atual + menu_largura - 1, menu_pos_y + 10, menu_pos_x_atual + menu_largura - 1, menu_pos_y + menu_altura - 10);
...
draw_rectangle(menu_pos_x_atual, menu_pos_y, menu_pos_x_atual + menu_largura, menu_pos_y + _header_height, false);

// Linha decorativa
draw_set_alpha(0.6);
draw_set_color(cor_borda_principal);
draw_line(menu_pos_x_atual + 10, menu_pos_y + _header_height - 2, menu_pos_x_atual + menu_largura - 10, menu_pos_y + _header_height - 2);

// Ícone do cabeçalho (hexágono grande)
var _header_icon_x = menu_pos_x_atual + 35;
...
if (menu_estado == 2) {
    ...
    draw_text_transformed(menu_pos_x_atual + menu_largura / 2, _rodape_y, "Pressione V ou ESC para fechar", _text_scale, _text_scale, 0);
}
