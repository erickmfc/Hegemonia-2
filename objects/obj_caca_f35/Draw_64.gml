...
if (selecionado) {
    ...
    draw_text(_text_x, _text_y, "F-35");
    _text_y += 18;
    ...
    // ✅ Linha 2: Status dos mísseis (F-35 usa sistema de mísseis múltiplos)
    var _missois_prontos = 0;
    if (variable_instance_exists(id, "timer_sky") && timer_sky <= 0) _missois_prontos++;
    if (variable_instance_exists(id, "timer_iron") && timer_iron <= 0) _missois_prontos++;
    if (variable_instance_exists(id, "timer_hash") && timer_hash <= 0) _missois_prontos++;
    if (variable_instance_exists(id, "timer_lit") && timer_lit <= 0) _missois_prontos++;
    
    if (_missois_prontos > 0) {
        draw_set_color(make_color_rgb(0, 255, 0)); // Verde limão
        draw_text(_text_x, _text_y, "Mísseis: " + string(_missois_prontos) + "/4");
    } else {
        draw_set_color(make_color_rgb(255, 165, 0)); // Laranja
        draw_text(_text_x, _text_y, "Mísseis recarregando");
    }
