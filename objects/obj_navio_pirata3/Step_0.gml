// ===============================================
// HEGEMONIA GLOBAL - NAVIO PIRATA TIPO 3 (Step)
...
if (!vinculado) {
    ...
    if (!variable_instance_exists(id, "eh_lider")) {
        ...
        if (array_length(_outros_piratas) == 0 || point_distance(0, 0, x, y) < point_distance(0, 0, _outros_piratas[0].x, _outros_piratas[0].y)) {
            eh_lider = true;
            show_debug_message("🏴‍☠️ Navio Pirata Tipo 3 é o LÍDER do esquadrão");
        }
    }
}
...
if (!eh_lider) {
    ...
    if (instance_exists(_lider)) {
        var _offset_angulo = 240;  // ✅ Tipo 3: 240 graus à direita
