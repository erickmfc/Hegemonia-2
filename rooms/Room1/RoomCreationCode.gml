...
// Cria uma instância do menu de ação
try {
    var _menu_acao_instance = instance_create_layer(0, 0, "rm_mapa_principal", obj_menu_de_acao);
    
    if (instance_exists(_menu_acao_instance)) {
        show_debug_message("Menu de Ação criado com sucesso na sala Room1. ID: " + string(_menu_acao_instance));
    } else {
        show_debug_message("ERRO: Falha ao criar Menu de Ação!");
    }
} catch (_error) {
    show_debug_message("AVISO: obj_menu_de_acao não registrado no projeto. Erro: " + string(_error));
    show_debug_message("Sistema de comandos desabilitado temporariamente.");
}
